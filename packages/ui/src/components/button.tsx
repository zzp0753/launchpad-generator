...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    { className, variant, size, asChild = false, icon, iconPosition = "left", isLoading, children, ...props },
    ref,
  ) => {
    const Comp = asChild ? Slot : "button";
    const IconComponent = isLoading ? Loader2 : icon;
    const showIcon = Boolean(IconComponent);

    const renderIcon = (position: "left" | "right") => {
      if (!showIcon || iconPosition !== position || !IconComponent) {
        return null;
      }
      ...
      return (
        <IconNode
          className={cn(position === "left" ? "mr-2 h-4 w-4" : "ml-2 h-4 w-4", isLoading && "animate-spin")}
          aria-hidden="true"
        />
      );
    };
